function experimentScript
    ...
    %We are going to make extensive use of an all-purpose set of functions
    %(helper functions). You'll notice them because the start with hf.*
    %They are located in the +hf folder.
    ...
    %Open mPigeon and wait for it to close
    waitfor(mPigeon);
    ...
    if isempty(expRunning); return; end %if control panel was quit, expRunning is empty;
        
    
    %Read csv, and make it into the trials
    trialsData = hf.makeTrials(expParameters.Task);
    
    screenInfo = struct();
    screenInfo.debug = 0; %only really useful for one-screen setups
    screenInfo.screenBG = [0, 0, 0, 1]; %screen background
    %initialize screen
    [screenInfo.windowPointer, screenInfo.displayRect] = hf.initScreen(screenInfo);
    
    %make textures from stimuli folder
    [screenInfo.textures, screenInfo.texturesKey] = hf.makeTextures(screenInfo);
    ...
    screenInfo.sampleRect = hf.rescaleRect(sampleRect, [0, 0, 1, 1], sqScreenRect);
    %Define option rects
    optRects = {[0, .40, .20, .60]...
                [.80, .40, 1, .60]};
    screenInfo.optRects = cellfun(@(x) hf.rescaleRect(x, [0, 0, 1, 1], sqScreenRect), optRects, 'uniformOutput', false);
    %Define start stimulus rect
    startRect = [.45, .45, .55, .55];
    screenInfo.startRect = hf.rescaleRect(startRect, [0, 0, 1, 1], sqScreenRect);
    ...
    saveInfo = hf.makeFilenames(saveInfo, expParameters);
    ...
    while expRunning
        ...
        if KbCheck
            sessionInfo.state = hf.abortCheck(sessionInfo.state); %Check if we want to abort the session (both ctrl keys)
            hf.showCursorCheck; %Check if we want to show the cursor (spacebar)
        end
        switch(sessionInfo.state)
                ...
                if any(buttons)
                    if IsInRect(x, y, screenInfo.sampleRect)
                        ...
                        if ~isfield(saveInfo, 'onPeckSaveFormat')
                            ...
                            %"%3.6"; default is %2.3f).
                            saveInfo.onPeckSaveFormat = hf.makeSaveFormat(trialInfo, saveInfo.onPeckSave, '\t', '%2.4\f');
                        end
                        hf.saveData(saveInfo.peckfname, saveInfo.onPeckSave, saveInfo.onPeckSaveFormat, trialInfo);
            ...
            case 'FEEDING'
                for p = 1:trialInfo.pellets
                    hf.silverbox(serialPort, 'feeder');
                end
            ...
            case 'END_TRIAL'
                ...
                if ~isfield(saveInfo, 'onTrialSaveFormat')
                    ...
                    %precision of floats as a fourth argument (e.g.,
                    %"%3.6".
                    saveInfo.onTrialSaveFormat = hf.makeSaveFormat(trialInfo, saveInfo.onTrialSave, '\t');
                end
                if trialInfo.firstAttempt
                    ...
                    hf.saveData(saveInfo.trialfname, saveInfo.onTrialSave, saveInfo.onTrialSaveFormat, trialInfo);
                end
                %save all attempts data
                hf.saveData(saveInfo.totalfname, saveInfo.onTrialSave, saveInfo.onTrialSaveFormat, trialInfo);
            ...
            case {'EXPERIMENT_END', 'ABORT'}
                ...
                if strcmp(sessionInfo.state, 'EXPERIMENT_END')
                    message = ['Session finished!', 10, sprintf('Session duration: %s', hf.toc2hm(sessionInfo.startTime)), 10, 10];
                    %read data file
                    sessData = hf.readData(strcat('./data/', saveInfo.trialfname), saveInfo.onTrialSave);
                    %use function to aggregate data and get summaries
                    message = [message, hf.getSummary(sessData, 'accuracy', 'sample')];
                else
...
function updateScreen(type, screenInfo, trialInfo, sessionInfo, expParameters)
    ...
    hf.drawParametersTop([1, 1, 1, 1], screenInfo, trialInfo, sessionInfo, expParameters);
    hf.drawParametersBottom([1, 1, 1, 1], screenInfo, trialInfo, sessionInfo, expParameters);
